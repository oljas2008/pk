...
if [ ! -d "$MAGIC_TIME_DIR" ]; then
    ...
    if [ ! -d "$MAGIC_TIME_DIR/Anykernel" ]; then
        git clone https://github.com/olzhas0986/Anykernel.git "$MAGIC_TIME_DIR/Anykernel"
...
if grep -q -E "Ошибка 2|Error 2" build.log; then
    ...
    cd "$MAGIC_TIME_DIR"
    7z a -mx9 PureLine-munch-$MAGIC_BUILD_DATE.zip * -x!*.zip
    
    curl -s -X POST https://api.telegram.org/bot$TGTOKEN/sendMessage \
    -d chat_id="@Ximipurekernel" \
    -d text="Kernel was compiled! Compile time: $elapsed_time second" \
    -d message_thread_id="2"

    curl -s -X POST "https://api.telegram.org/bot$TGTOKEN/sendDocument?chat_id=@Ximipurekernel" \
    -F document=@"./PureLine-munch-$MAGIC_BUILD_DATE.zip" \
    -F caption="PureLine (trying to add susfs) branch: ${BRANCH}" \
    -F message_thread_id="2"
